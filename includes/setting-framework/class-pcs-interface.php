 ...
 * @license   GPL-2.0+
 */
class Pcs_Framework_Interface {

    /**
     * Generates the tabs that are used in the options menu
     */
    static function pcs_framework_tabs() {
        $counter = 0;
        $options = & Pcs_Framework::_pcs_framework_options();
        $menu = '';
        if ($options):
     ...
     */
    static function pcs_framework_fields() {

        global $allowedtags;

        // Gets the unique option id
        $option_name = pc_setting_name();
        ...
        foreach ($options as $value) {
            ...
            switch ($value['type']) {
                ...
                case "images":
                    ...
                    break;
                case "icons":
                    $pc_networks = Pc_Social_Share_Core::pc_get_networks();
                    $name = $option_name . '[' . $pc_id . ']';
                    foreach ($value['options'] as $key => $option) {
                        $selected = '';
                        if ($val != '' && ($val == $key)) {
                            $selected = ' of-radio-img-selected';
                        }
                        $output .= '<div class="wpslw_icon_name_wrapper">';
                        $output .= '<input type="radio" id="' . esc_attr($pc_id . '_' . $key) . '" class="of-radio-img-radio" value="' . esc_attr($key) . '" name="' . esc_attr($name) . '" ' . checked($val, $key, false) . ' />';
                        $output .= '<div class="of-radio-img-label">' . esc_html($key) . '</div>';
                        $output .= '</div>';
                        $output .= '<div class="social-sharing theme-' . $key . ' of-radio-img-img' . $selected . '" onclick="document.getElementById(\'' . esc_attr($pc_id . '_' . $key) . '\').checked=true;" >';
                        if ($pc_networks):
                            foreach ($pc_networks as $pc_networks):
                                $pc_network_id = $pc_network['id'];
                                $pc_name = $pc_network['name'];
                                $pc_url = $pc_network['url'];
                                $output .= '<a target="_blank" title="' . $pc_name . '" href="' . $pc_url . '?u=http://www.jqueryscript.net" class="share-' . $pc_network_id . ' tester">';
                                $output .= '<span class="icon icon-' . $pc_network_id . '"></span>';
                                $output .= '<span class="share-title">Share</span>';
                                $output .= '<span class="share-count is-loaded">1171</span>';
                                $output .= '</a>';
                            endforeach;
                        endif;
                        $output .= '</div>';
                ...
                case 'typography':
                    ...
                    $typography_options = array(
                        'sizes' => pcs_of_recognized_font_sizes(),
                        'faces' => pcs_of_recognized_font_faces(),
                        'styles' => pcs_of_recognized_font_styles(),
                        'color' => true
                    );
                    ...
                    $typography_fields = apply_filters('pcs_of_typography_fields', $typography_fields, $typography_stored, $option_name, $value);
                ...
                case 'background':
                    ...
                    $repeats = pcs_of_recognized_background_repeat();
                    ...
                    $positions = pcs_of_recognized_background_position();
                    ...
                    $attachments = pcs_of_recognized_background_attachment();
        ...
        if (Pcs_Framework_Interface::pcs_framework_tabs() != '') {
            echo '</div>';
        }
    }
